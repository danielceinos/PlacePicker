...
class PlacePickerActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private lateinit var placeAcceptButton: FloatingActionButton
    private lateinit var placeCenterButton: FloatingActionButton
    ...
    private val searchRequestCode = 700
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        showCoordinatesTextView(showLatLong)
        markerImage = findViewById(R.id.marker_image_view)
        markerShadowImage = findViewById(R.id.marker_shadow_image_view)
        placeAcceptButton = findViewById(R.id.place_chosen_button)
        placeCenterButton = findViewById(R.id.place_center_button)
        searchButton = findViewById(R.id.place_search_button)

        placeAcceptButton.setOnClickListener {
            if (addresses != null) {
        ...
        placeCenterButton.setOnClickListener {
        ...
        searchButton.setOnClickListener {
            val intent = Intent(this, PlacePickerSearchActivity::class.java)
    ...
    private fun getIntentData() {
        latitude =
            intent.getDoubleExtra(Constants.INITIAL_LATITUDE_INTENT, Constants.DEFAULT_LONGITUDE)
        longitude =
    ...
    private fun setIntentCustomization() {
        ...
        if (fabColorRes != -1) {
            placeAcceptButton.backgroundTintList =
                ColorStateList.valueOf(ContextCompat.getColor(this, fabColorRes))
        }
        if (primaryTextColorRes != -1) {
            setPrimaryTextColor(ContextCompat.getColor(this, primaryTextColorRes))
        }
        if (secondaryTextColorRes != -1) {
            setSecondaryTextColor(ContextCompat.getColor(this, secondaryTextColorRes))
        }
    }
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        fusedLocationClient.lastLocation.addOnSuccessListener { location: Location? ->
            if (location == null || latitude != Constants.DEFAULT_LATITUDE || longitude != Constants.DEFAULT_LONGITUDE)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == searchRequestCode && resultCode == Activity.RESULT_OK && data != null) {
            setAddress("${data.getStringExtra(PLACE_TITLE_KEY)} ${data.getStringExtra(PLACE_SUBTITLE_KEY)}")
        }
    ...
    private fun setAddress(addressName: String) {
        ...
        try {
            ...
            return if (addresses != null && addresses.size != 0) {
                ...
                fullAddress = address.getAddressLine(0) // If any additional address line present than only, check with max available address lines by getMaxAddressLineIndex()
                shortAddress = generateFinalAddress(fullAddress).trim()
                map.moveCamera(CameraUpdateFactory.newLatLngZoom(LatLng(address.latitude, address.longitude), zoom))
            } else {
                shortAddress = ""
                fullAddress = ""
            }
    ...
    fun setPlaceDetails(latitude: Double,
                        longitude: Double,
                        shortAddress: String,
                        fullAddress: String) {
